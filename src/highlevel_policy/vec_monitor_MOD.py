...
class VecMonitor(VecEnvWrapper):
    ...
    def reset(self,indices=None) -> VecEnvObs:
        ...
        #self.episodic_proxy = np.zeros(self.num_envs, dtype=np.float32)
        return obs
    ...
    def step_wait(self,indices=None) -> VecEnvStepReturn:
        ...
        obs, rewards, dones, infos = self.venv.step_wait(indices)
            #remove reward here since it is successively increased even though the env is inactive
            #below the done function will take the reward again and again because done is always true
           # print("interVEC",rewards)
        ...
        for i,indice in enumerate(indices):
            ...
            if dones[indice]:
                ...
                if self.results_writer:
                    #print("WRITER:{}-{} {}".format(i,indice,infos[i]['success']),episode_info)
                    #input()
                    self.results_writer.write_row(episode_info)
                #print("WROTE RESULTS for : ",indice, " and active : ",indices)
                ...
                #print("ETRXA INFOS:",new_infos[indice])
        #print("VEC MON")
